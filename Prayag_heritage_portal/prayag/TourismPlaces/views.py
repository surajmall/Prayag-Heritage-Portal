...
def home(request):
	page_quote = 'Imerse yourself in some of the inspiring destinations in Allahabad'
...
def stay_visit(request):
    return render(request, 'prayag_stay/stay_visit.html', {})

def temp_events(request):
	place = TourismPlace.objects.all().filter(place_type__contains="temp_event")[0]
	print(place.name)
	related_images = Image.objects.all().filter(tourism_place__place_type__contains="temp_event")
